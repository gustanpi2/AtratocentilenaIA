    ...
    <div className="relative w-14 h-14">
      {[0, 1, 2].map((i) => (
        <div
          ...
          style={{ opacity: 0, animation: `mrv-pulse 1.8s ease-out ${i * 0.6}s infinite` }}
        />
      ))}
...
const GoogleMapComponent: React.FC = () => {
  ...
  const focusLocation = useCallback((loc: Location) => {
    mapRef.current?.panTo({ lat: safeParse(loc.lat), lng: safeParse(loc.lng) });
  ...
  const heatmapData = isLoaded
    ? filtered.filter((l) => l.lat && l.lng).map((l) => new google.maps.LatLng(safeParse(l.lat), safeParse(l.lng)))
    : [];
  ...
  return (
    <div className="relative w-full h-full overflow-hidden" style={{ minHeight: "400px" }}>
      ...
      {/* Global keyframes */}
      <style>{`
        @keyframes mrv-pulse { 0%{opacity:.8;transform:scale(.5)} 100%{opacity:0;transform:scale(2.2)} }
        @keyframes mrv-blink { 0%,100%{opacity:1} 50%{opacity:.3} }
      `}</style>
        ...
        >
          {heatmapVisible && heatmapData.length > 0 && (
            <HeatmapLayer
              data={heatmapData}
              options={{
                radius: 100, opacity: 0.35, dissipating: true,
                gradient: [
                  "rgba(132,204,22,0)", "rgba(132,204,22,0.6)",
                  "rgba(200,240,60,0.8)", "rgba(250,200,20,1)",
                  "rgba(255,120,0,1)", "rgba(239,68,68,1)",
                ],
              }}
            />
          )}
            ...
            locGroup.map((location, i) => {
              ...
              return (
                <Marker
                  key={`${location.id}-${i}`}
                  position={{
                    lat: safeParse(location.lat) + offset * Math.cos(angle),
                    lng: safeParse(location.lng) + offset * Math.sin(angle),
                  }}
                  icon={{ url: "/images/IconGeo.png", scaledSize: new window.google.maps.Size(40, 40) }}
          ...
          <div className="absolute top-3 left-3 right-3 z-30 flex items-center gap-2">
            ...
            <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border backdrop-blur-xl shrink-0 ${panelCls}`}>
              <span
                ...
                style={{ boxShadow: "0 0 8px rgba(132,204,22,0.8)", animation: "mrv-blink 2s ease-in-out infinite" }}
              />
          ...
          <div className="absolute bottom-6 right-4 z-30 flex flex-col gap-2">
            ...
            <button
              ...
              className={`flex items-center justify-center w-9 h-9 rounded-xl border backdrop-blur-xl cursor-pointer transition-all duration-200
                ${heatmapVisible
                  ? "bg-lime-500/20 border-lime-500/50 text-lime-400 shadow-[0_0_16px_rgba(132,204,22,0.3)]"
                  : isDark ? "bg-slate-900/90 border-slate-700/40 text-slate-400 hover:text-lime-400" : "bg-white/90 border-slate-200 text-slate-500 hover:text-lime-600"
                }`}
            >
              <Thermometer size={16} />
            </button>
                    ...
                    >
                      ...
                      <div className="flex gap-1.5 flex-wrap">
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded-full border font-mono ${
                          ...
                          {safeParse(loc.lat).toFixed(4)}, {safeParse(loc.lng).toFixed(4)}
                        </span>
                      </div>
                    </div>
                  );
